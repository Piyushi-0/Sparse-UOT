...
save_in = f"LOGS_s{args.s}/{args.out}_{args.data}_{args.sure_and_possible}"
...
if __name__ == '__main__':
    ...
    if args.best_thresh is not None:
        ...
        lda3_list = [0.1, 0, 1e-3, 1e-2]
        ktype_list = ['rbf', 'imq_v2', 'imq']
        khp_list = ['med_8', '16_med', '8_med', '4_med']
        lda_list = [0.1, 1, 10]

    for lda3 in lda3_list:
        for ktype in ktype_list:
            for khp in khp_list:
                for lda in lda_list:
                    ...
                    hps = f"{lda}_{lda3}_{khp}_{ktype}_{max_itr}_{s}_{ws}_{K}_{keuc}_{all_gamma}"
                    ...
                    if args.best_thresh is not None: # NOTE: post validation run the test
