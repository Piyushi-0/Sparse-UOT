...
parser.add_argument('--out', default='bsl/uns', type=str)
...
save_in = f"{args.out}_{args.data}_{args.sure_and_possible}"
os.makedirs(save_in, exist_ok=True)

logger = createLogHandler(f"{save_in}/{args.save_as}.csv", str(os.getpid()))
logger.info("ot_type, best_th, best_f1, best_pr, best_re, best_ex") #, weight_type, dist_type, div_type, chimera, pair_encode, centering, layer, seed")
...
if __name__ == '__main__':
    ...
    for ot_type in args.ot_type:
        out_dir = os.path.join(save_in, ot_type)
        ...
        if ot_type == 'uot' or (ot_type == 'pot' and args.sinkhorn):
            ...
            logger.info(f"{ot_type}, {best_thresh}, {best_log['total_f1']}, {best_log['total_precision']}, {best_log['total_recall']}, {best_log['total_exact_match']}")
