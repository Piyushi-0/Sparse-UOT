...
class MoCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            mu = np.full((n,), expected_num_tokens_per_expert)
            nu = np.ones((m,), dtype=C.dtype)
            
            regul = SparsityConstrained(gamma=self.gamma, max_nz=1)
            if self.v == 1:
                alpha, res = ot.smooth.solve_semi_dual(mu, nu, C, regul, max_iter=self.max_iter)
                # assert res.success
                
                pi = np.transpose(ot.smooth.get_plan_from_semi_dual(alpha, nu, C, regul))
            else:
                alpha, beta, res = ot.smooth.solve_dual(mu, nu, C, regul, max_iter=self.max_iter)
                # assert res.success
                pi = np.transpose(ot.smooth.get_plan_from_dual(alpha, beta, C, regul))
            
            index = torch.from_numpy(np.argmax((pi>0).astype(np.int32), axis=1)).to(device) # NOTE: This is for top-1.
        # mobilenet
