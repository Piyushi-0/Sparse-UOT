...
def get_gamma(C, G1, G2, v1, v2, max_itr, lda, K=None, s=0, seed=0, ws=0,
              all_gamma=0):
    ...
    Args:
        ...
        K (None or _int_): None if cardinality constraint m*n else
...
def get_gamma_dash(C, G1, G2, v1, v2, max_itr, lda, seed, allg,
    ...
    if neg_max_grd >= 0:
        return [gamma0] if allg else gamma0
    for k in range(tot):
        if k:
            # R: V_minus_L
...
def get_gamma_sdash(C, G1, G2, v1, v2, max_itr, lda, s, seed,
    ...
    for k in range(tot):
        ...
        if k:
            ...
            g1_new = g1_new.index_copy(0, unq_i_R, g1)
            g2_new = g2_new.index_copy(0, unq_j_R, g2)
            vec_grd = (fixed_grd+2*lda*(g1_new[:, None]
                                        + g2_new)
                                        ).take(R)
            u_0 = torch.argmin(vec_grd)
            neg_max_grd = vec_grd[u_0]
...
def get_gamma_dash_ws(C, G1, G2, v1, v2, max_itr, lda, seed, allg,
    ...
    neg_max_grd = vec_grd[u_0]
    if neg_max_grd >= 0:
        return [gamma] if allg else gamma
    for k in range(tot):
        if k:
            S_i_R = torch.div(V_minus_L, n, rounding_mode='floor')
            S_j_R = V_minus_L % n
